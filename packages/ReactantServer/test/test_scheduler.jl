...
_sched_trivial_manifest(name) = Manifest(
    "2.0", name, "", TensorSpec[], TensorSpec[], nothing,
...
function _sched_batched_manifest(name; features = 2)
    ...
    return Manifest(
        "2.0", name, "", TensorSpec[inx], TensorSpec[outy], nothing, nothing,
...
@testset "priority: spec worked example" begin
    ...
    @test pa ≈ -0.0278 atol = 1.0e-4
    @test pb ≈ 0.00493 atol = 1.0e-4
    @test pc ≈ 0.00259 atol = 1.0e-4
...
@testset "EMA decay halves over a half-life" begin
    ...
    @test st.recent_compute_ema ≈ 0.5 atol = 1.0e-9
    _decay_ema!(st, 30.0, 60.0)               # a second half-life
    @test st.recent_compute_ema ≈ 0.25 atol = 1.0e-9
end

@testset "cost estimate converges toward measurement" begin
    st = ModelSchedState("m", ModelSchedConfig(1.0), 0.0)
    _update_cost!(st, 1, 0.04, 0.2)          # first measurement seeds the estimate
    @test st.cost_estimate[1] == 0.04
    for _ in 1:50
        _update_cost!(st, 1, 0.01, 0.2)
    end
    @test st.cost_estimate[1] ≈ 0.01 atol = 1.0e-3
    @test effective_cost(st, 1, 0.1) ≈ 0.01 * 0.9 atol = 1.0e-4
end
...
@testset "select_dispatch! picks the fair winner (worked example)" begin
    s = _empty_sched()
    now = 1000.0
    specs = (("A", 0.7, 0.01), ("B", 0.2, 0.03), ("C", 0.1, 0.1))
