...
# memory probe (scheduler.jl), both exercised without a GPU: the cache's layout, hash record,
# invalidation and atomic writes are pure filesystem logic, and the probe is driven against a mock
# backend with a simulated allocator. The GPU half (PJRT C API compile, serialize, load) is covered
# by test/spike_pjrt_capi.jl on a CUDA host.
...
@testset "executable cache: counters and backend defaults" begin
    ...
    @test supports_executable_cache(ReactantServer.PJRTCAPIBackend())
    @test clear_memory_stats!(mock, pool) === false
    @test compiled_memory_stats(mock, ReactantServer.MockExecutable(x -> x, 1)) === nothing
    @test ReactantServer.PJRTCAPI.availability() isa Tuple{Bool, String}
end
