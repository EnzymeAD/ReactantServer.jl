...
_obs_manifest(name) = Manifest(
    "2.0", name, "",
    TensorSpec[
        TensorSpec("x", F32, Dim[Dim(FIXED, 3), Dim(FIXED, 224), Dim(BATCH)], 3),
...
_obs_entry(name, model) = ModelEntry(
    name, _obs_manifest(name), Dict{Int, Vector{UInt8}}(),
