...
function host_materialize(
        store::SharedWeightStore, key, st, names::Vector{String};
    ...
    return materialize_host_weights!(
        store, key, weights_digest(String(key), specs; content = content),
        specs, fill!
    )
end
