...
function Prometheus.collect!(metrics::Vector, c::WorkerSnapshotCollector)
    ...
    push!(
        ...
        Prometheus.Metric(
            "counter", "worker_rows_served_total",
        ...
        Prometheus.Metric(
            "gauge", "worker_model_max_batch_size",
    ...
    push!(
        metrics, Prometheus.Metric(
            "gauge", "worker_queue_wait_seconds",
    ...
    push!(
        ...
        _scalar(
            "worker_resident_weight_bytes", "gauge",
    ...
    if wc !== nothing
        push!(
            ...
            _scalar(
                "worker_weight_load_seconds_total", "counter",
    ...
    if dm !== nothing
        push!(
            ...
            _scalar(
                "worker_device_memory_limit_bytes", "gauge",
                "Device memory pool limit (bytes).", dm.limit
            ),
            _scalar(
                "worker_device_memory_free_bytes", "gauge",
                "Device bytes free to allocate.", dm.free
            ),
            _scalar(
                "worker_device_memory_peak_in_use_bytes", "gauge",
                "Peak device bytes in use since startup (allocator high-water mark).", dm.peak_in_use
            ),
            _scalar(
                "worker_device_memory_pool_bytes", "gauge",
...
function inject_metric_labels(text::AbstractString, labels::Vector{Pair{String, String}})
    ...
    for line in eachline(IOBuffer(text); keep = true)
        ...
        if brace !== nothing && (sep === nothing || brace < sep)
            ...
            add = join(
                (
                    "$k=\"$(_escape_label_value(v))\"" for (k, v) in labels
                        if !occursin("$k=\"", existing)
                ), ","
            )
            write(
                out, isempty(add) ? line :
...
server to stop it.
"""
function start_worker_metrics(
        m::WorkerMetrics, host::AbstractString, port::Integer; ready_fn,
