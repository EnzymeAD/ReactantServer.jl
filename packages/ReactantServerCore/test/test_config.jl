...
function load_single_worker(
        dir, global_body::AbstractString;
        base_port::Int = 8080,
...
@testset "config load + env overrides" begin
    mktempdir() do dir
        modeldir = joinpath(dir, "models"); mkpath(modeldir)
        cfg, applied, wname = load_single_worker(
            dir, """
            runtime:
              backend: cpu
              mem_fraction: 0.8
        ...
        withenv(
            "INFERENCE_SERVER_SCHEDULER_EMA_HALFLIFE_SECONDS" => "30",
            "INFERENCE_SERVER_ENDPOINTS_PORT" => "12345",
            "INFERENCE_SERVER_RUNTIME_BACKEND" => "cuda"
        ) do
            cfg2, applied2, _ = load_single_worker(
                dir, """
                runtime:
                  backend: cpu
                endpoints:
        ...
        @test_throws ReactantServer.ConfigError load_single_worker(
            dir, """
            batch_policy:
              max_batch_size: 8
            """; model_repo = modeldir
...
@testset "scheduler config: defaults, per-model overrides, validation" begin
    mktempdir() do dir
        ...
        @test cfg.scheduler.coalescing_discount == 0.2
        ...
        @test cfgb.scheduler.coalescing_discount == 0.1
        ...
        for (key, val) in (
                ("recency_penalty_cap", "1.5"), ("coalescing_discount", "1.0"),
                ("cost_ema_alpha", "0"),
            )
...
@testset "weight cache fraction + wiggle: parse, env, validation, disable" begin
    mktempdir() do dir
        ...
        withenv(
            "INFERENCE_SERVER_RUNTIME_WEIGHT_CACHE_FRACTION" => "0.75",
            "INFERENCE_SERVER_RUNTIME_WEIGHT_CACHE_WIGGLE_FRACTION" => "0.2"
        ) do
...
@testset "grpc config: defaults, node-global parse, env override, validation" begin
    mktempdir() do dir
        ...
        # Environment overrides win over the file.
        withenv(
            "INFERENCE_SERVER_GRPC_MAX_RECV_MSG_BYTES" => "4096",
            "INFERENCE_SERVER_GRPC_MAX_SEND_MSG_BYTES" => "8192"
        ) do
...
@testset "model_control_mode: parse, default, residency, validation, migration" begin
    mktempdir() do dir
        ...
        cdyn, _, _ = load_single_worker(
            dir, "model_control_mode: dynamic\nmodel_poll_seconds: 5.0";
            model_repo = modeldir
        )
        ...
        cbadpoll, _, _ = load_single_worker(
