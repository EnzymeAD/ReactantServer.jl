...
function extract_block(t, kw)
    ...
    lb = findnext('[', t, m.offset); (o, c) = _matched_span(t, lb, '[', ']')
    return t[nextind(t, o):prevind(t, c)]
end
...
struct IOE
    name::String; dtype::String; dims::Vector{Int}
end
function parse_io(t, kw)
    ...
    for obj in extract_objects(blk)
        ...
        nm === nothing && continue
        dims = Int[]; dm !== nothing && for tok in split(dm.captures[1], ',')
            s = strip(tok); isempty(s)||push!(dims, parse(Int, s))
        end
...
function julia_to_torch(arr::AbstractArray{T}) where {T}
    ...
    return torch.from_numpy(na.copy())
end
torch_to_julia(t) = (
...
struct Res
    model::String; batch::Int; relerrs::Vector{Float64}; ok::Bool; note::String
end
...
function check_model(bundle::String)
    ...
    batches = sort(
        [
            parse(Int, match(r"model\.b(\d+)\.mlir", f).captures[1])
    ...
    port = _free_port()
    cfgsrv = ReactantServer.ServerConfig(
        [root], "",
    ...
    try
        ...
        for n in batches
            ...
            resp = HTTP.post(
                "$base_url/v2/models/$bundle/infer",
...
function _append_results(rs::Vector{Res})
    return open(RESULTS_TSV, "a") do io
        for r in rs
            errs = join([string(e) for e in r.relerrs], ",")
            println(
                io, join(
                    [
                        r.model, string(r.batch), string(r.ok), errs,
                        replace(r.note, '\t' => ' ', '\n' => ' '),
                    ], '\t'
                )
            )
